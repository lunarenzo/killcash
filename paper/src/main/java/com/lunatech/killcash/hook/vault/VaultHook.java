package com.lunatech.killcash.hook.vault;
 ...
 */
public class VaultHook extends AbstractHook implements Listener, EconomyProvider {
     ...
     */
    public VaultHook(KillCash plugin) {
        super(plugin);
    }
    ...
    public boolean isLoaded() {
        return isEconomyLoaded();
    }

    @Override
    public double getBalance(OfflinePlayer player) {
        if (!isEconomyLoaded()) return 0.0;
        return getEconomy().getBalance(player);
    }

    @Override
    public boolean deposit(OfflinePlayer player, double amount) {
        if (!isEconomyLoaded()) return false;
